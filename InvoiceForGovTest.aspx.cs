...
public partial class InvoiceForGovTest : System.Web.UI.Page
{
...
Logger.EmptyLog("item.InnerUse_AgencyHotelId = " + item.InnerUse_AgencyHotelId, eLogger.INVOICE_XML_ERROR);
    ...
    private List<CLReferral> fillReferral(string[] param, ClAsciFile item, out  decimal PriceForChecktmp, out decimal CoDiscountamnt)
    {
        ...
        if (Convert.ToInt32(param[32]) == 3) //uom is hotel (hotel - 3, other - 27)
        {
            ...
            for (int i = 0; i < Convert.ToInt16(item.NightNum); i++)
            {
				...
				string priceId = DAL_SQL.GetRecord("HOTELS", "hotel_price_id", item.InnerUse_BundleId, "bundle_id");
				bool isNewPrices = (priceId == "-1"); //if -1 then its new prices.
				int dayIndex = i;
                Price = GetPriceAndPriceZakai(item, makattmp, 1, StartDate.AddDays(nightCount), peoplecount, Convert.ToInt16(item.HotelMelave2.Substring(i, 1)), pricesByDay, out priceZakai, vatPercent.ToString(), isNewPrices, dayIndex);

				Logger.Log("Price = " + Price + ", priceZakai = " + priceZakai, eLogger.EXCEPTION);
    ...
    private decimal GetPriceAndPriceZakai(ClAsciFile item, string makattmp, int p1, DateTime CurrentDate, int peoplecount, short p2, Dictionary<int, DaysPrices> daysPrices, out decimal priceZakai, string iVatPercant, bool isNewPrices, int dayIndex)
    {
        double PriceTotal = 0;
		decimal PriceTotalDec = 0;
        DaysPrices dp = new DaysPrices();
        priceZakai = 0;
		...
		if (isNewPrices)
		{
			try
			{
				Logger.Log("bundleId = " + item.InnerUse_BundleId, eLogger.EXCEPTION);
				...
				string baseId = "0";
				string areaId = "0";
				string supplierId = item.InnerUse_AgencyHotelId.ToString();
				string compositionId;
				string result, resultMelaveSubsid;
				string fromDate, toDate;
				List<string> results = new List<string>();			
				
				fromDate = CurrentDate.ToString("dd-MMM-yy");
				toDate = CurrentDate.AddDays(1).ToString("dd-MMM-yy");
				
				if (item.InnerUse_Erkev_Type_Name == "ZakaiAndMelaveBeTashlum")
				{
					Logger.Log("ZakaiAndMelaveBeTashlum.", eLogger.EXCEPTION);
					XmlDocument finalPricexXmlSingle = new XmlDocument();
					XmlDocument finalPricexXmlMelaveSubsid = new XmlDocument();
					
					compositionId = "1"; //single
					Logger.Log("agencyId = " + agencyId + ", systemType = " + systemType + ", clerkId = " + clerkId + ", iAreaId = " + areaId + ", iSupplierId = " + supplierId + ", priceType = " + priceType + ", langStr = " + langStr + ", iFromDate = " + fromDate + ", iToDate = " + toDate + ", compositions = " + compositionId + ", baseId = " + baseId, eLogger.EXCEPTION);
					result = priceWs.getHotelPricePerAreaWithoutSaleCycle(agencyId, systemType, clerkId, 
												  areaId, supplierId, priceType, langStr, 
												  fromDate, toDate, 
												  compositionId, baseId);

					Logger.Log("0result = " + result, eLogger.EXCEPTION);
					compositionId = "2330";
					Logger.Log("agencyId = " + agencyId + ", systemType = " + systemType + ", clerkId = " + clerkId + ", iAreaId = " + areaId + ", iSupplierId = " + supplierId + ", priceType = " + priceType + ", langStr = " + langStr + ", iFromDate = " + fromDate + ", iToDate = " + toDate + ", compositions = " + compositionId + ", baseId = " + baseId, eLogger.EXCEPTION);
					resultMelaveSubsid = priceWs.getHotelPricePerAreaWithoutSaleCycle(agencyId, systemType, clerkId, 
												  ...
												  fromDate, toDate, 
												  compositionId, baseId);
					Logger.Log("resultMelaveSubsid = " + resultMelaveSubsid, eLogger.EXCEPTION);
					finalPricexXmlSingle.LoadXml(result);
					finalPricexXmlMelaveSubsid.LoadXml(resultMelaveSubsid);			
					if (finalPricexXmlMelaveSubsid.SelectNodes("Root//FinalPrices//FinalPrice").Count > 0 && 
						finalPricexXmlSingle.SelectNodes("Root//FinalPrices//FinalPrice").Count > 0) //has result
					{				
						decimal price = 0;
						string iMelaveDates = getMelaveDatesStr(item.InnerUse_BundleId, item);
						XmlDocument docToGetPriceFrom;
						
						if (iMelaveDates[dayIndex] == '1')	
						{
						...
						foreach (XmlNode node in docToGetPriceFrom.SelectSingleNode("//Root//FinalPrices//"))
						{
							if (node.SelectSingleNode("//FinalPrice//HasError").InnerText != bool.TrueString)
							{
								price = decimal.Parse((decimal.Parse(node.SelectSingleNode("//FinalPrice//FinalPriceBrutto").InnerText) / (1 + (decimal.Parse(iVatPercant)/ 100))).ToString());
				...
				{
					Logger.Log("regular invoice.", eLogger.EXCEPTION);
					foreach (int priceForErkev in item.InnerUse_Erkev_Type)
					{
						compositionId = priceForErkev.ToString();
						Logger.Log("agencyId = " + agencyId + ", systemType = " + systemType + ", clerkId = " + clerkId + ", iAreaId = " + areaId + ", iSupplierId = " + supplierId + ", priceType = " + priceType + ", langStr = " + langStr + ", iFromDate = " + fromDate + ", iToDate = " + toDate + ", compositions = " + compositionId + ", baseId = " + baseId, eLogger.EXCEPTION);
						result = priceWs.getHotelPricePerAreaWithoutSaleCycle(agencyId, systemType, clerkId, 
														  areaId, supplierId, priceType, langStr, 
														  fromDate, toDate, 
														  compositionId, baseId);
						Logger.Log("1result = " + result, eLogger.EXCEPTION);
						XmlDocument finalPricexXml = new XmlDocument();
						finalPricexXml.LoadXml(result);

						if (finalPricexXml.SelectNodes("Root//FinalPrices//FinalPrice").Count > 0) 
						{							
							//foreach (XmlNode node in finalPricexXml.SelectNodes("Root//FinalPrices//FinalPrice"))
							for (int i = 0; i < finalPricexXml.SelectNodes("Root//FinalPrices//FinalPrice").Count; i++)
							{
								XmlNode node = finalPricexXml.SelectNodes("Root//FinalPrices//FinalPrice")[i];
								Logger.Log("here2 = " + node.SelectNodes("//HasError")[i].InnerText, eLogger.EXCEPTION);
								Logger.Log("rooType = " + node.SelectNodes("//RoomType//Name")[i].InnerText, eLogger.EXCEPTION);
								
								if (node.SelectNodes("//FinalPrice//HasError")[i].InnerText != bool.TrueString)
								{
									Logger.Log("here3", eLogger.EXCEPTION);
									PriceTotalDec += decimal.Parse((decimal.Parse(node.SelectNodes("//FinalPriceBrutto")[i].InnerText) / (1 + (decimal.Parse(iVatPercant)/ 100))).ToString());
									priceZakai += decimal.Parse((decimal.Parse(node.SelectNodes("//FinalPriceBrutto")[i].InnerText) / (1 + (decimal.Parse(iVatPercant)/ 100))).ToString());
								}
							}
							
							Logger.Log("PriceTotalDec = " + PriceTotalDec, eLogger.EXCEPTION);
							Logger.Log("priceZakai = " + priceZakai, eLogger.EXCEPTION);
						}
					}
				}
			}
			...
			{
				Logger.Log("Exception in getPrice from new prices. Exception = " + ex.Message, eLogger.EXCEPTION);
				Logger.Log("trace = " + ex.StackTrace, eLogger.EXCEPTION);
			}
			
			PriceTotal = double.Parse(PriceTotalDec.ToString());
		}
		else
		{
	...
	public string getMelaveDatesStr(string bundleId, ClAsciFile item)
	{
		string iMelaveDates = DAL_SQL.GetRecord("BUNDLES", "melave_selected_nights", bundleId ,"id");
    ...
    public DataSet getPricesFromDB(ClAsciFile item)
    {
	  ...
	  //Logger.EmptyLog(getHotelPricesSql, eLogger.INVOICE_XML_ERROR);
        ...
        if (!string.IsNullOrEmpty(priceId))
        {
			...
			Logger.Log("priceId = " + priceId, eLogger.EXCEPTION);
