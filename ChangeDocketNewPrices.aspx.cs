...
public partial class ChangeDocketNewPrices : System.Web.UI.Page
{
	...
	{		
		AgencyPricesSearch agn = new AgencyPricesSearch();
		//Brutto:11.11|Netto:22.22|BruttoCouple:33.33
		string agencyId = "85";
		string systemType = "3";
		string priceId = "-1";
		string roomType = "2";
		string fromDate = iBundleRow.mFromDate.ToString(dateFormat);
		string toDate = iBundleRow.mToDate.ToString(dateFormat);
		DateTime fromDateDT = DateTime.Parse(fromDate);
        DateTime toDateDT = DateTime.Parse(toDate);
		int nights = (toDateDT - fromDateDT).Days;
		string melaveDates = "".PadLeft(nights, '0');
		string supplierId = iBundleRow.mPayToSupplier.ToString();
		string bruttoNettoCouple = agn.GetPriceByPriceIdAndRoomTypesAndDates(agencyId, systemType, priceId, 
																			 roomType, fromDate, toDate, 
																			 melaveDates, supplierId);
	}
    ...
    private string getBundlesSql()
    {
        ...
        string sql = string.Format(@"
               SELECT *  
               FROM BUNDLES AS B INNER JOIN VOUCHERS V ON V.bundle_id = B.id 
					INNER JOIN HOTELS H ON H.bundle_id = B.id
               WHERE B.from_date >= cast('01-Apr-17' as smalldatetime)
               AND V.status = 1 AND V.service_type_id = {0}
				   H.hotel_price_id = -1
